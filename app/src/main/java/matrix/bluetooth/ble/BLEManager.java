...
import matrix.bluetooth.ConnectActivity;
import matrix.bluetooth.util.ClsUtils;
...
public class BLEManager  {
    ...
    private String serviceUUID,readUUID,writeUUID, readUUIDData;
    private BluetoothGattService bluetoothGattService;   //服务
    private BluetoothGattCharacteristic readCharacteristic;  //读特征
    private BluetoothGattCharacteristic readCharacteristicData;  //读特征
    ...
    private BluetoothGattCallback bluetoothGattCallback = new BluetoothGattCallback() {
        ...
        public void onServicesDiscovered(BluetoothGatt gatt, int status) {
            ...
            if(setupService(gatt,serviceUUID,readUUID,writeUUID, readUUIDData)){
                if(onBleConnectListener != null){
        ...
        public void onCharacteristicRead(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic, int status) {
            ...
            Log.w(TAG, "onCharacteristRead: " + TypeConversion.bytes2HexString(bytes,bytes.length)
                    + " UUID: " + characteristic.getUuid().toString());

            if (onBleConnectListener != null) {
                onBleConnectListener.onReadMessage(gatt, gatt.getDevice(), characteristic, bytes);
            }
     ...
     */
    @RequiresApi(api = Build.VERSION_CODES.JELLY_BEAN_MR2)
    public BluetoothGatt connectBleDevice(Context context, BluetoothDevice bluetoothDevice, long outTime,String serviceUUID,String readUUID,String writeUUID, String readUUID1, OnBleConnectListener onBleConnectListener){
        if(bluetoothDevice == null){
     ...
     */
    @RequiresApi(api = Build.VERSION_CODES.JELLY_BEAN_MR2)
    private boolean setupService(BluetoothGatt bluetoothGatt,String serviceUUID,String readUUID,String writeUUID, String readUUIDData) {
        ...
        //这里检测蓝牙设备的service, 来匹配自己的ServiceUUID, 所以蓝牙设备一定要提供匹配的Service，并且自己要写相应ServiceUUID
        ...
        if(readUUID == null || writeUUID == null || readUUIDData == null){
        ...
        for (BluetoothGattCharacteristic characteristic : bluetoothGattService.getCharacteristics()) {
            ...
            } else if (characteristic.getUuid().toString().equals(readUUIDData)) {
        ...
        if (readCharacteristicData == null) {
            Log.e(TAG, "setupService()-->readCharacteristicData == null");
            return false;
        }
        ...
        enableNotification(true, bluetoothGatt, readCharacteristicData);
        List<BluetoothGattDescriptor> descriptors2 = readCharacteristicData.getDescriptors();
...
//        mBluetoothGatt.close();
    ...
    public void readDevice() {
        if (mBluetoothGatt == null) {
            Log.e(TAG, "readDevice-->bluetoothGatt == null");
            return;
        }
